...
{
    const int rows = settings()->value("rows", 1).toInt();
    const bool show_only_active = settings()->value("showOnlyActive", false).toBool();
    const int label_type = settings()->value("labelType", DesktopSwitchButton::LABEL_TYPE_NUMBER).toInt();
